...
/// <summary>
/// Performance monitor for vision system operations in Diagnostics context.
...
public sealed class VisionPerformanceMonitor : IVisionPerformanceMonitor
{
    ...
    private readonly ConcurrentDictionary<EntityId, VisionStats> _actorStats;
    ...
    public VisionPerformanceMonitor(ILogger<VisionPerformanceMonitor> logger)
    {
        ...
        _actorStats = new ConcurrentDictionary<EntityId, VisionStats>();
    }
    ...
    public void RecordFOVCalculation(EntityId actorId, double calculationTimeMs, int tilesVisible, int tilesChecked, bool wasFromCache)
    {
    ...
    public Fin<VisionPerformanceReport> GetPerformanceReport()
    {
            ...
            lock (_statsLock)
            {
                ...
                if (allMetrics.Length == 0)
                {
                    return new VisionPerformanceReport(
                        ...
                        ActorStats: new Dictionary<EntityId, VisionStats>()
                    );
                }
                ...
                var report = new VisionPerformanceReport(
                    ...
                    ActorStats: new Dictionary<EntityId, VisionStats>(_actorStats)
                );

                _logger.LogInformation("Vision performance report (Diagnostics): {Calculations} calculations, {CacheHitRate:P1} cache hit rate, {AverageMs:F2}ms average",
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Error generating vision performance report (Diagnostics)");
    ...
    public void Reset()
    {
        ...
        _actorStats.Clear();
        _logger.LogDebug("Vision performance monitor reset (Diagnostics)");
    }

    private void UpdateActorStats(EntityId actorId, VisionMetric metric)
    {
